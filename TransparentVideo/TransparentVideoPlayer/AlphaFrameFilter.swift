...
typealias AlphaFrameFilterError = AlphaFrameFilter.ErrorType
...
final class AlphaFrameFilter: CIFilter {
    ...
    private(set) var outputError: ErrorType?
    ...
    func process(inputImage: CIImage, mask maskImage: CIImage) throws -> CIImage {
        self.inputImage = inputImage
        self.maskImage = maskImage
        
        guard let outputImage = self.outputImage else {
            throw outputError ?? ErrorType.unknown
        }
    ...
    private static var metalKernelError: ErrorType?
    ...
    private func render(using renderingMode: RenderingMode, inputImage: CIImage, maskImage: CIImage) -> CIImage? {
        ...
        case .builtInFilter:
            guard let filter = CIFilter(name: "CIBlendWithMask") else {
                outputError = ErrorType.buildInFilterNotFound
                return nil
            }
        ...
        case .colorKernel:
            ...
            guard let colorKernel = AlphaFrameFilter.colorKernel else {
                outputError = ErrorType.invalidKernel
                return nil
            }
        ...
        case .metalKernel:
            guard let metalKernel = AlphaFrameFilter.metalKernel else {
                outputError = AlphaFrameFilter.metalKernelError ?? ErrorType.invalidKernel
                return nil
            }
            let outputExtent = inputImage.extent
